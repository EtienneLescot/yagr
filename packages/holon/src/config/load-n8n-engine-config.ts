...
function missingFieldError(field: string): Error {
  return new Error(
    `Missing required n8n workspace configuration field: ${field}. Initialize the workspace before starting Holon.`,
  );
}
...
export async function loadN8nEngineConfig(configService = new ConfigService()): Promise<N8nEngineConfig> {
  ...
  if (!apiKey) {
    throw new Error(
      `Missing API key for ${localConfig.host}. Authenticate with n8n-as-code before starting Holon.`,
    );
  }
