...
import { HolonRunEngine } from './runtime/run-engine.js';
import type {
  DeployedWorkflow,
  GeneratedWorkflow,
  HolonRunOptions,
...
export class HolonAgent {
  ...
  async run(prompt: string, options: HolonRunOptions = {}): Promise<HolonRunResult> {
    const runner = new HolonRunEngine(this.engine, this.history);
