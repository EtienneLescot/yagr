...
import type { HolonContextCompactionEvent, HolonLanguageModelConfig, HolonRunJournalEntry } from '../types.js';
...
export interface CompactConversationInput {
  ...
  llmConfig?: HolonLanguageModelConfig;
  condense?: (prompt: string) => Promise<string>;
}

export interface CompactConversationResult {
  messages: CoreMessage[];
  event?: HolonContextCompactionEvent;
}
...
function sanitizeTranscriptText(text: string): string {
  ...
  if (trimmed.startsWith('Holon internal phase:')) {
    return '';
  }
...
function buildJournalDigest(prompt: string, journal: HolonRunJournalEntry[]): string {
...
function buildFallbackSummary(prompt: string, journal: HolonRunJournalEntry[], compactedMessages: CoreMessage[]): string {
...
function buildSummaryPrompt(prompt: string, journal: HolonRunJournalEntry[], compactedMessages: CoreMessage[]): string {
  return [
    'You are condensing Holon runtime context for continuation inside the same task.',
    ...
    'Output plain text only. Start with the heading "## Holon Context Checkpoint".',
    '',
    'Journal digest:',
    buildJournalDigest(prompt, journal),
    '',
