...
import { getYagrAgentMemorySources, getYagrAgentSkillSourcePaths } from '../dist/agent-factory.js';
...
test('agent factory skill sources are delegated to Deep Agents paths (array)', () => {
  const sources = getYagrAgentSkillSourcePaths({ includeEmpty: true });
  assert.ok(Array.isArray(sources), 'skill sources should be an array');
  for (const src of sources) {
    assert.equal(typeof src, 'string', 'each source should be a string path');
  }
});
