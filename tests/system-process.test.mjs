...
test('resolveNativeShell keeps POSIX-compatible configured shells on Unix', () => {
  const previousShell = process.env.SHELL;
  process.env.SHELL = '/bin/zsh';
  try {
    assert.equal(resolveNativeShell('linux').file, '/bin/zsh');
    assert.equal(resolveNativeShell('linux').args.at(-1), '-c');
  } finally {
...
test('resolveNativeShell falls back to sh when configured shell is non-POSIX', () => {
  const previousShell = process.env.SHELL;
