...
import { buildRelayInfo, ensureN8nRelayServer, getN8nRelayState, N8N_RELAY_CREDENTIAL_NAME, N8N_RELAY_FAKE_API_KEY, } from '../llm/llm-relay-server.js';
 ...
 */
async function runN8nacCommand(args, cwd) {
...
async function listYagrProxyCredentials(cwd) {
 ...
 */
async function patchN8nCredentialUrl(credentialId, newBaseUrl, cwd) {
...
export async function ensureYagrProxyCredential() {
    ...
    const existing = existingCredentials.find((c) => c.name === N8N_RELAY_CREDENTIAL_NAME && c.type === 'openAiApi');
    if (existing?.id) {
    ...
    const createResult = await runN8nacCommand(['credential', 'create', '--type', 'openAiApi', '--name', N8N_RELAY_CREDENTIAL_NAME, '--data', credData, '--json'], cwd);
    const created = parseJsonPayload(createResult.stdout);
 ...
 */
export async function syncProxyCredentialIfEnabled() {
    const configService = new YagrConfigService();
    if (!configService.getLocalConfig().llmProxy?.enabled)
        return;
...
export function createYagrProxyTool(observer) {
    return new DynamicStructuredTool({
        name: 'yagrProxy',
        description: 'Reads the current Yagr LLM proxy status and returns the provisioned openAiApi credential when available. '
...
//# sourceMappingURL=yagr-proxy.js.map