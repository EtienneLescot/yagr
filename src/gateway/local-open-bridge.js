...
function readLocalOpenBridgeState() {
    try {
...
function saveLocalOpenBridgeState(state) {
    ensureYagrHomeDir();
    fs.writeFileSync(getLocalOpenBridgeStatePath(), JSON.stringify(state, null, 2));
}
function clearLocalOpenBridgeState() {
    try {
...
function getActiveLocalOpenBridgeState() {
...
function spawnLocalOpenBridgeProcess() {
    ensureYagrHomeDir();
...
async function waitForLocalOpenBridgeState(timeoutMs) {
...
export async function stopLocalN8nAuthBridge() {
    const state = readLocalOpenBridgeState();
    if (!state) {
        return;
    }
