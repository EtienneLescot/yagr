...
export { getWebUiGatewayStatus } from './webui-config.js';
...
export function createWebUiGatewayRuntime(options = {}, configService = new YagrConfigService()) {
...
class WebUiGateway {
    ...
    constructor(options, configService, status) {
    ...
    async handleRequest(request, response) {
        ...
        if (method === 'POST' && url.pathname === '/api/chat') {
            ...
            const result = await agent.invoke({ messages: [{ role: 'user', content: message }] }, this.sessions.buildSessionConfig(sessionId));
        ...
        if (method === 'POST' && url.pathname === '/api/slash') {
            ...
            const slashCtx = { surface: 'webui', sessionId, threadId: sessionId };
            const webuiHandler = {
                getActiveSessionId: () => this.sessions.getActiveForScope({ kind: 'webui', key: sessionId })?.id,
                resumeSession: (_scope, resumeSessionId) => {
        ...
        if (method === 'PATCH' && url.pathname.startsWith('/api/sessions/')) {
            ...
            const body = await this.readJson(request);
            const displayThread = body.displayThread;
    ...
    persistSessionMetadata(sessionId) {
        const session = this.sessions.readDisplaySession(sessionId);
        this.sessions.persistMemory(sessionId, session?.title ?? 'New conversation', session?.createdAt ?? new Date().toISOString());
    }
    ...
    async sendBinaryAsset(response, fileName, contentType) {
        const assetPath = path.resolve(__dirname, '..', 'webui', fileName);
    ...
    async handleStreamingChat(response, sessionId, message) {
        ...
        try {
            ...
            const stream = agent.streamEvents({ messages: [{ role: 'user', content: message }] }, { ...this.sessions.buildSessionConfig(sessionId), signal: abortController.signal });
            const lastProgressKeys = new Set();
            ...
            for await (const event of stream) {
                ...
                if (DEBUG_AGENT_LOOP) {
                    ...
                    const eventType = 'event' in event ? event.event : 'unknown';
...
function isCompactionState(value) {
    return Boolean(value
        ...
        && typeof value.totalCompactions === 'number');
}
