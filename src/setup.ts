...
export async function runYagrSetup(
  ...
  if (!n8nCompleted) {
    p.cancel('Yagr setup cancelled during n8n setup.');
    return false;
  }

  const llmCompleted = await runLlmSetup(yagrConfigService);
  if (!llmCompleted) {
    p.cancel('Yagr setup cancelled during LLM setup.');
    return false;
  }

  const surfacesCompleted = await runSurfaceSetup(yagrConfigService);
  if (!surfacesCompleted) {
    p.cancel('Yagr setup cancelled during gateway setup.');
    return false;
  }

  const status = getYagrSetupStatus(yagrConfigService, n8nConfigService);
  const n8nLocalConfig = n8nConfigService.getLocalConfig();
  const yagrLocalConfig = yagrConfigService.getLocalConfig();
...
async function runN8nSetup(configService: N8nConfigService): Promise<boolean> {
  ...
  try {
    ...
    if (projects.length === 0) {
      ...
      throw new Error('No n8n projects were found. Create one in n8n first, then rerun Yagr setup.');
    }
  } catch (error) {
...
async function runLlmSetup(configService: YagrConfigService): Promise<boolean> {
  const currentConfig = configService.getLocalConfig();
...
async function promptForProvider(initialProvider?: YagrModelProvider): Promise<YagrModelProvider | undefined> {
  if (initialProvider) {
  ...
  const selection = await p.select<YagrModelProvider>({
    ...
    options: VALID_PROVIDERS.map((provider) => ({
      value: provider,
      label: provider,
      hint: 'Configured and persisted by Yagr setup',
    })),
  });
