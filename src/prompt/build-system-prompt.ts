...
export function buildSystemPromptSnapshot(engine: EngineIdentityPort): SystemPromptSnapshot {
  ...
  return {
    systemPrompt: [
      ...
      'When configuring an LLM credential on an AI/LangChain node, trust llm_provider_options as the source of truth. Only recommend providers whose metadata says available=true. For Yagr Proxy (frictionless, no API key needed): call yagr_proxy_relay_start — it starts the relay server and creates the openAiApi credential automatically; just use the returned credentialId.',
