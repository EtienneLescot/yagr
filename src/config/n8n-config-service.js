...
import { ConfigService, createFallbackInstanceIdentifier, createProjectSlug, resolveInstanceIdentifier, } from 'n8nac';
 ...
 */
export function resolveWorkflowDir(config) {
...
function preferEnvironmentCredentials(env) {
    return /^(1|true|yes|on)$/i.test(String(env.YAGR_PREFER_ENV_CREDENTIALS || '').trim());
}
export function resolveN8nRuntimeState(configService, env = process.env, options = {}) {
    ...
    const projectConfigured = Boolean(host
...
export class YagrN8nConfigService {
    ...
    constructor() {
        ...
        this.compatibilityStore = new Conf({
            projectName: 'n8nac',
     ...
     */
    syncN8nacHostUrl(tunnelPublicUrl) {
        ensureYagrHomeDir();
        const workspaceDir = getYagrN8nWorkspaceDir();
        const n8nacConfigPath = path.join(workspaceDir, 'n8nac-config.json');
        if (!fs.existsSync(n8nacConfigPath)) {
        ...
        try {
            ...
            const currentRootHost = typeof config.host === 'string'
                ? config.host?.replace(/\/+$/, '') ?? ''
                : '';
