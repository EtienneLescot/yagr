  ...
  const injectMemoryTool = tool(
    async ({ path: filePath }: { path: string }) => {
      ...
      if (injected.has(resolved)) {
        return (
          `✅ Already active: "${resolved}" is already injected and governing this session. ` +
      ...
      try {
        ...
        return (
          `✅ Injected: "${resolved}" (${content.length} chars) is now active for this session. ` +
          `Do NOT call inject_memory again — proceed directly with the user's request.`
        );
      } catch (err) {
    ...
    },
    {
      ...
      description:
        'Inject an AGENTS.md or instruction file into persistent session memory. ' +
