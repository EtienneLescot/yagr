...
export type WorkflowSpecConnections = WorkflowSpecConnection[] | Record<string, Record<string, Array<Array<{
    node: string;
    type: string;
    index?: number;
...
export type YagrAgentState = 'idle' | 'running' | 'streaming' | 'stopped' | 'waiting_for_permission' | 'waiting_for_input' | 'compacting' | 'resumable' | 'completed' | 'failed_terminal';
...
export interface YagrRuntimeHook {
    ...
    afterTool?: (context: YagrToolHookContext & {
        result: unknown;
    }) => void | Promise<void>;
    beforeCompletion?: (attempt: YagrCompletionAttempt, context: YagrRuntimeContext) => void | YagrCompletionHookDecision | Promise<void | YagrCompletionHookDecision>;
}
...
export type YagrOperationCategory = 'file-read' | 'file-write' | 'shell' | 'web' | 'tool' | 'agent' | 'phase' | 'thinking';
