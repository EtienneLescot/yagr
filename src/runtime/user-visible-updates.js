...
function extractWorkflowEmbedFromText(raw) {
...
export function makePhaseOperationEvent(event) {
...
// ---------------------------------------------------------------------------
export function makeToolStartOperationEvent(toolName, rawInput) {
    ...
    switch (toolName) {
        ...
        default: {
            if (!toolName)
                return undefined;
            const detail = input ? Object.values(input).find((v) => typeof v === 'string') : undefined;
            return {
                kind: 'operation',
                operationId,
                label: toolName,
...
function looksLikeYagrProxyCommand(command) {
    return /(^|\s)(?:npx\s+)?yagr\s+yagrProxy(\s|$)/.test(command.trim());
}
// ---------------------------------------------------------------------------
// Tool-end → operation event update
// ---------------------------------------------------------------------------
export function makeToolEndOperationEvent(operationId, toolName, rawOutput, startedAt) {
    const ended = Date.now();
    const base = { status: 'done', endedAt: ended };
    if (toolName === 'execute') {
...
function rawOutputToString(raw) {
    if (typeof raw === 'string')
        return raw;
...
export function makeThinkingEndEvent(body, startedAt) {
...
export function mapPhaseEventToUserVisibleUpdate(event) {
...
export function mapStateEventToUserVisibleUpdate(event) {
    switch (event.state) {
...
export function mapToolEventToUserVisibleUpdate(event) {
...
//# sourceMappingURL=user-visible-updates.js.map