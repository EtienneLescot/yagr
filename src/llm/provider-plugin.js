...
function getMiniMaxCompletionDiscoveryUrl(baseUrl) {
...
async function probeMiniMaxModels(apiKey, baseUrl) {
    const url = getMiniMaxCompletionDiscoveryUrl(baseUrl);
    const checks = await Promise.all(MINIMAX_DISCOVERY_CANDIDATE_MODELS.map(async (model) => {
    ...
    return checks.reduce((models, model) => {
        if (model) {
            models.push(model);
        }
        return models;
...
function buildProviderPlugin(provider) {
...
function buildProviderDiscovery(provider, definition) {
...
const providerPluginCache = new Map();
