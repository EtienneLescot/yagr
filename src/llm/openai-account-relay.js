...
import { translateChatCompletionToResponsesApi } from './responses-api-relay.js';
...
function buildRelayCodexUserAgent() {
    return 'codex_cli_rs/0.0.0 (Unknown 0; unknown) unknown';
}
function getRelaySessionId(payload) {
...
function getRelayInstallationId() {
...
function translateChatCompletionsToCodex(payload) {
    let instructions = '';
    const input = [];
    ...
    const tools = Array.isArray(payload.tools) && payload.tools.length > 0
        ? payload.tools.map((t) => ({
            ...
            parameters: normalizeFunctionToolParametersSchema((t.function.parameters ?? { type: 'object', properties: {} }), { forceRequiredObjectProperties: true }),
            strict: true,
        }))
        : [];
...
function createCodexSseState(model) {
    return {
...
function makeChunk(state, delta, finishReason) {
 ...
 */
function translateCodexSseEvent(event, state) {
    ...
    if (type === 'response.output_item.added') {
        const item = (typeof event.item === 'object' ? event.item : event.output_item);
...
function buildNonStreamingResponse(state) {
    const message = {
        role: 'assistant',
 ...
 */
export async function handleOpenAiAccountRelay(_req, res, body, options) {
    ...
    if (isStreaming) {
        if (options?.fromResponsesApi) {
            ...
            try {
                for await (const event of parseCodexSSE(upstream.body)) {
...
//# sourceMappingURL=openai-account-relay.js.map