 ...
 */
const PROVIDERS_WITH_NATIVE_RESPONSES_API = new Set(['openai', 'openrouter']);
...
let activeServer;
// ─── State persistence ────────────────────────────────────────────────────────
export function getN8nRelayState() {
    try {
 ...
 */
export async function resolveDockerHostAddress() {
 ...
 */
export async function ensureN8nRelayServer() {
...
export function buildRelayInfo(port) {
...
function spawnRelayProcess() {
    ...
    const entrypoint = path.resolve(path.dirname(fileURLToPath(import.meta.url)), 'llm-relay-entrypoint.js');
...
async function waitForRelayState(timeoutMs) {
 ...
 */
export async function ensureN8nRelayServerInProcess() {
    const existing = getN8nRelayState();
    if (existing && isRelayAlive(existing)) {
        return buildRelayInfo(existing.port);
    }
 ...
 */
export function closeN8nRelayServerInProcessForTests() {
    if (activeServer) {
        try {
...
async function attemptListen(server, port) {
...
async function startRelayInProcess() {
...
async function resolveProviderRuntime() {
    ...
    const runtime = await prepareProviderRuntime(resolved.provider, {
...
function injectModel(body, model) {
    try {
...
function extractResponsesTextContent(content, type) {
...
export function translateResponsesRequestToChatCompletionsBody(body) {
    let payload;
    try {
...
async function handleChatCompletions(req, res, fromResponsesApi = false) {
    const result = await resolveProviderRuntime();
    ...
    if (fromResponsesApi && !PROVIDERS_WITH_NATIVE_RESPONSES_API.has(provider)) {
        ...
        const upstreamChat = await fetch(chatUrl, {
            ...
            body: chatBody.buffer.slice(chatBody.byteOffset, chatBody.byteOffset + chatBody.byteLength),
        });
        ...
        if (contentType.includes('text/event-stream')) {
            ...
            const chatCompletion = (await upstreamChat.json());
    ...
    const upstream = await fetch(targetUrl, {
        method: 'POST',
        headers: forwardHeaders,
        body: bodyWithModel.buffer.slice(bodyWithModel.byteOffset, bodyWithModel.byteOffset + bodyWithModel.byteLength),
    });
...
//# sourceMappingURL=llm-relay-server.js.map