...
export function getGitHubCopilotSession() {
...
export async function resolveCopilotApiToken(githubToken) {
...
export async function validateGitHubCopilotRuntime(modelId = GITHUB_COPILOT_DEFAULT_MODEL) {
...
export async function fetchGitHubCopilotModels(token, baseUrl = DEFAULT_COPILOT_API_BASE_URL) {
...
export async function beginGitHubCopilotAuth() {
...
function importGitHubCliSession() {
...
function extractYamlTopLevelBlock(content, key) {
...
function writeStoredGitHubSession(session) {
    const sessionPath = getGitHubSessionPath();
    fs.mkdirSync(path.dirname(sessionPath), { recursive: true });
    fs.writeFileSync(sessionPath, JSON.stringify(session, null, 2));
}
function getCopilotTokenCachePath() {
...
function readCachedCopilotToken(cachePath) {
...
function writeCachedCopilotToken(cachePath, token) {
...
//# sourceMappingURL=copilot-account.js.map