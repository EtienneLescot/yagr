# Holon Agentic TUI UX Direction
...
These are the decisions Holon should follow.
...
- side rail: compact timeline of what Holon has already done
...
- Holon should therefore be frame-first at the top level, while still exposing integrated scrollback on demand

### 2. There Should Be Four Message Lanes, Not One

Every emitted event should map to one lane:

- narrative: what Holon understands and intends to do
...
The user should feel that Holon is translating terminal noise into operational meaning.
...
If Holon needs permission or user input, that event should visually take over the interface.
...
Holon should not look like a generic green-on-black chat terminal.
...
- include one short sentence about what Holon can do
- include 3 prompt starters
- include the key shortcuts in a light, low-noise ribbon

## Message Transmission Rules

These rules should govern how Holon talks to the user in the TUI.
...
- Holon wordmark or compact identity mark
- session title
- state badge
- phase badge
- model badge
...
Holon should include a dedicated in-app history surface.
...
- The user should never need to parse logs to know whether Holon is healthy.
...
That is the highest-leverage change for making Holon feel like a premium agent console rather than a styled transcript.